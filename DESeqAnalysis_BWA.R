...
plotPDFs=F
fileNamePrefix="BWA_"
filterPrefix="BWA_rptOnly_random_"
dataset="_random"
filterData=T
...
for (grp in c(controlGrp,groupsOI)){
  ...
  idx<-colData(dds)$strain %in% c(grp)
...
###### filter genes
if(filterData){
  oscillating<-read.delim(paste0(outPath,"/oscillatingGenes_latorre.tsv"),header=T,
                          stringsAsFactors=F)
...
for(grp in groupsOI){
  res<-results(dds,contrast=c("strain",grp,controlGrp))
  ...
  resLFC<-lfcShrink(dds,coef=paste0("strain_",grp,"_vs_",controlGrp), type="apeglm", res=res)
  class(resLFC)
  ### add metadata
  ...
  if(! filterData){
    resLFC$worbaseID<-as.vector(metadata$wormbaseID)[idx]
  ...
  resLFC$overlap<-as.vector(metadata$overlap)[idx]
  ...
  for (g in selectedGenes) {
    barplot(counts(dds, normalized=TRUE)[g,],
            col=colData(dds)$strain,
            main=g, las=2, cex.names=1,names.arg=colData(dds)$sampleName)
    legend("topleft",legend=levels(colData(dds)$strain),fill=c(1,2), cex=0.7)
  }
  dev.off()
  ...
  mcols(resGR)<-resLFC[,c("ID","log2FoldChange","padj")]
  ...
  plotMA(res, main=paste0(grp," uncorrected LFC, threshold=", padjVal), ylim=c(-3,3), alpha=padjVal,colLine="#66666633")
  plotMA(resLFC, main=paste0(grp," apeglm shrunk LFC, threshold=", padjVal), ylim=c(-3,3), alpha=padjVal,colLine="#66666633")
  ...
  sigUp<-sum(resLFC$padj<padjVal & resLFC$log2FoldChange>lfcVal)
  sigDown<-sum(resLFC$padj<padjVal & resLFC$log2FoldChange< -lfcVal)
  p2<-EnhancedVolcano(resLFC,
                      lab=paste0(resLFC$rptID,"\n",
  ...
  if(plotPDFs==T){
    ggsave(filename=paste0(outPath,"/plots/",fileNamePrefix, grp,
                           "_volcanoPlot_allGenes1.pdf"), plot=p2,
           device="pdf",path=outPath, width=12,height=12,units="cm")
  } else {
    ggsave(filename=paste0(outPath,"/plots/",fileNamePrefix, grp,
                           "_volcanoPlot_allGenes1.png"), plot=p2,
           device="png",path=outPath, width=12,height=12,units="cm")
  }
