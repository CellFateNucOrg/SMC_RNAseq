...
The results tables from this final run are what were used in the paper. The reason for this is that the *dpy-26*cs sample leads to upregulation of so many X-linked genes that it violates the assumptions of the DESeq2 model that most genes do not change, and so the model compensates for so many X-linked genes being upregulated by considering most autosomal genes as downregulated. Something that is clearly not the case when you analyse the autosomes on their own.

### Advanced contrasts

We initially performed the analysis separately on different batches: the klesin cleavage samples and the degron samples (both of which included *dpy-26*cs cleavage), but to avoid having two different sets of numbers for significant genes for the same *dpy-26*cs sample due to slight variations of the DESeq2 model for each batch, we performed the final analysis for all the samples together (the results were similar). As our range of RNAseq conditions did not completely cover all possible combinations of conditions (kleisin cleavage and auxin treatment) we created a variable composed of the combinations seen in our data, by combining the status for which kleisin with a cut site was present, presence of TIR protease, presence of *sdc-3*AID, and auxin treatment into a single "SMC" factor variable with 11 levels:
...
The basic DESeq2 model was built with the PWM366 sample as the reference "control" sample. However for the degron samples this was not always the most appropriate control, e.g. for the PMW821 strain (hs::TEV, *sdc-3*AID, P*eft-3*::TIR1) we wanted to contrast the +- auxin conditions. Advanced contrasts also helped us examine conditional effects such as the effect of *sdc-3*AID in the presence of *dpy-26*cs. These contrasts are optionally created towards the end of the variableSetting.R script by subtracting coefficients of models built with the different levels of the SMC variable above. The contrast naming scheme uses the same positional variable scheme as the SMC variable above, but an X is placed in the position of the conditions that vary, and the specific subtraction/contrast is written after an \_ at the end of the contrast name. Although not all were used in the paper, the following contrasts were examined:
