...
for i in ${biolRep[@]}
do
	repFiles=( `ls ${WORK_DIR}/tracks/avr_*_UniqueMultiple_RPM.bw | grep ${i}` )
	echo summing F and R  strands: ${repFiles[@]}

	bigWigMerge ${repFiles[@]} ${WORK_DIR}/tracks/sumFR_${i}_UniqueMultiple.bedGraph

	bedGraphToBigWig ${WORK_DIR}/tracks/sumFR_${i}_UniqueMultiple.bedGraph $chromSizesFile ${WORK_DIR}/tracks/sumFR_${i}_UniqueMultiple.bw

	if [ -f ${WORK_DIR}/tracks/sumFR_${i}_UniqueMultiple.bw ]; then
		rm ${WORK_DIR}/tracks/sumFR_${i}_UniqueMultiple.bedGraph
	fi
done
