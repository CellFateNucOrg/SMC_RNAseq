...
# using mRNAs as decoys (bad idea?)
...
# using genome as decoys
grep "^>" <(gunzip -c  ${GENOME_DIR}/sequence/c_elegans.PRJNA13758.${genomeVer}.genomic.fa.gz) | cut -d " " -f 1 > ${GENOME_DIR}/sequence/decoys.txt
sed -i.bak -e 's/>//g' ${GENOME_DIR}/sequence/decoys_forRepeats.txt


cat  ${GENOME_DIR}/annotation/repeats_ce11_dfam_nr.fa.gz ${GENOME_DIR}/sequence/c_elegans.PRJNA13758.${genomeVer}.mRNA_transcripts.fa.gz ${GENOME_DIR}/sequence/c_elegans.PRJNA13758.${genomeVer}.genomic.fa.gz > ${GENOME_DIR}/sequence/repeats_gentrome.fa.gz
